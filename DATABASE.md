...
- `0128_payment_methods.sql` — `payment_methods` (saved card-on-file **tokens, never card numbers**; ADR-043). The `provider_token` column is **service-role-only**: column-level GRANTs to `authenticated` cover brand/last4/expiry but **exclude `provider_token`**, so the browser can list a card yet never read its token (charging is server-side). RLS owner read/delete; no client insert/update. `set_default_payment_method(uuid)` SECURITY DEFINER (ownership-checked) + BEFORE-INSERT auto-default trigger (mirrors vehicles 0041). Adds `orders.paid_at`/`payment_ref`/`payment_method_id` (nullable; `paid_at` lets washer-visibility be gated on payment when the live charge is wired). INERT until the `save-card`/`charge-saved-card` Edge Fns deploy + `VITE_ENABLE_PAYMENTS=true`. Pinned by `savedCards.contract.test.jsx`.
...
**One-time setup (done 2026-06-12 unless noted):** Vault secret `send_receipt_url` ✓; `send-receipt` deployed ✓; `TRIGGER_SECRET` ✓; `RESEND_API_KEY` ✓; end-to-end verified ✓. **Done (verified 2026-06-15):** the real domain `muluwash.com` is verified in Resend; its DNS was moved to **Cloudflare** with the Resend records intact (`send.` SPF + `feedback-smtp` MX, `resend._domainkey` DKIM) under a strict `p=reject` DMARC that passes via DKIM alignment; `receipt_sender_email` = `receipts@muluwash.com` and עוסק מורשה/business details are set. End-to-end re-verified by invoking the deployed `send-receipt` function (`{sent:true}`, real email delivered to the customer inbox). Same date as initial deploy: `delete-account` + `fan-out-legal-update` were found **never deployed** and are now deployed, with Vault `fan_out_legal_update_url` created.

### Receipt retention — NONE (keep forever)

Receipt **rows AND their archived PDFs are retained permanently**. The PDFs in the private `receipts` bucket are the **original חשבונית מס/קבלה documents the business files with the tax authority each month**, so they must never be auto-deleted (regenerating a copy later is not the same as the filed original). `delete-account` already exempts the `receipts` bucket (0114), and there is **no time-based purge** of receipts.

Migration `0122` briefly introduced a 6-month PDF purge (cron → Edge Fn `purge-receipt-pdfs`); this was a mistake and is **fully reverted by `0123`**: cron `purge-receipt-pdfs` unscheduled, functions `purge_receipt_pdfs_tick` / `list_purgeable_receipt_pdfs` / `mark_receipt_pdfs_purged` dropped, the `pdf_purged_at` column dropped. Out-of-band (not schema): the `purge-receipt-pdfs` Edge Function was undeployed and the Vault secret `purge_receipt_pdfs_url` removed. **Do not reintroduce a receipt purge.** (Unrelated: the 90-day *photo* purge in 0115 stands — photos are not financial records.)

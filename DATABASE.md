...
### Receipt PDF retention (0122)

The receipt **row is a permanent tax record** (Israeli law ~7 years; 0115 already exempts receipts from the photo purge). 0122 reclaims **storage only**: a daily `pg_cron` tick (`purge_receipt_pdfs_tick`, 03:30 UTC) → `net.http_post` → Edge Fn `purge-receipt-pdfs` deletes the archived PDF from the private `receipts` bucket ~6 months (`retention_days`, default 180) after `created_at`, then nulls `pdf_path` + stamps `pdf_purged_at` via `mark_receipt_pdfs_purged`. Candidates come from `list_purgeable_receipt_pdfs` (service_role only; idempotent — a nulled `pdf_path` is never re-selected). Nothing financial is lost: `admin_resend_receipt` regenerates an identical PDF from the row snapshot, and the admin Receipts tab shows a "PDF purged" hint where the download used to be. Pinned by `receiptPdfRetention.contract.test.js`.

**One-time setup (DONE 2026-06-15):** migration `0122` applied (functions + `pdf_purged_at` + cron `purge-receipt-pdfs` @ `30 3 * * *`, active) ✓; Edge Fn `purge-receipt-pdfs` deployed ✓; Vault `purge_receipt_pdfs_url` created ✓ (`TRIGGER_SECRET` was already set project-wide, shared with the other fan-outs) ✓. Smoke-tested ✓ — authorized invoke returns `{receipts:0, nothing to purge}` (no receipts past the window yet), unauthorized returns 401.

...
The app has two visual modes. Originally role-based (consumer always light, washer dark), the split was relaxed in ADR-023 (consumers can opt into dark) and then **defaulted to light for everyone in ADR-044** — dark is now opt-in only, with no role-based default.

| Mode | Who | Trigger | Palette style |
|------|-----|---------|---------------|
| **Light (default, all roles)** | Everyone unless they opt out | Default when `display_preference` unset | Consumer: concrete Tailwind classes (`neutral-*`, `primary-*`). Washer: semantic token classes (`text-ink`, `bg-surface`, `border-edge`) |
...
| **Washer (dark, opt-in)** | Washers who prefer dark | `display_preference = 'dark'` via Washer Settings | Same semantic token classes |

As of ADR-044, `profile.display_preference` drives theme for both roles and the default is **light for every role** when `display_preference` is unset. Dark is only ever applied on an explicit `display_preference = 'dark'`.
...
| Job Detail | `/washer/job/:id` | washer | light / dark | `WasherShell` |
| Earnings | `/washer/earnings` | washer | light / dark | `WasherShell` |
| Settings | `/washer/settings` | washer | light / dark | `WasherShell` |
...
Theme is resolved in this priority order (`src/lib/resolveTheme.js`):
1. **Explicit user preference** — `display_preference` field on the `profiles` table
2. **Default — light for every role** (ADR-044). No role-based dark default.
3. **System `prefers-color-scheme`** — fallback only in the brief window before a profile has loaded

-- Switch approval workflow from admin role to agent role.
-- The 'admin' enum value stays in profiles_role_check (dead but harmless).

-- ── 1. Drop admin-scoped policies from 0019 ──────────────────────────────────
DROP POLICY IF EXISTS "Admins can read all orders"   ON public.orders;
DROP POLICY IF EXISTS "Admins can read all profiles" ON public.profiles;
DROP POLICY IF EXISTS "Admins can read all evidence" ON storage.objects;

-- ── 2. Recreate as agent-scoped ───────────────────────────────────────────────
...
CREATE POLICY "Agents can read all evidence"
...
-- ── 3. Update transition_order_status: pending_approval → completed ───────────
-- Preserved verbatim from 0019. Only change: v_actor_role = 'admin' → 'agent'
-- on the pending_approval → completed transition.
...
BEGIN
  ...
  -- pending_approval → completed (agent only; sets approved_at / approved_by)

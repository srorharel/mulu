-- Fix: migration 0032 accidentally reproduced transition_order_status from 0023,
-- overwriting the 0031 body that uses completion_photo_* / arrival_photo_* checks.
-- This restores the correct 0031 logic and adds the payout_amount write at acceptance.
...
BEGIN
  ...
  -- Agent complete override: complete from any non-terminal status
  ...
  UPDATE public.orders SET
    status                = new_status,
    washer_id             = CASE WHEN new_status = 'accepted'
                                 THEN auth.uid()  ELSE washer_id             END,
    accepted_at           = CASE WHEN new_status = 'accepted'
    ...
    -- Lock in washer's tier-based payout at the moment of acceptance
    payout_amount         = CASE WHEN new_status = 'accepted'
                                 THEN public.payout_for_tier(

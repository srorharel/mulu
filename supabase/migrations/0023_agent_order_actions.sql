-- Extend transition_order_status to allow agents to cancel and force-complete orders.
-- Preserved verbatim from 0020 except:
--   1. Added agent-cancel block (any non-terminal status).
--   2. Added agent-complete override block (any non-terminal status, bypasses evidence/pending_approval).
--   3. approved_at / approved_by now stamp on ANY agent-complete (not only from pending_approval).
...
BEGIN
  ...
  UPDATE public.orders SET
    ...
    -- Stamp approved_at/approved_by whenever an agent completes (normal or override path).

-- Migration 0083: transition_order_status gains a p_admin_override boolean.
--
-- When true AND the caller is super_admin, photo/GPS/geofence checks are
-- skipped and any non-terminal → any-status transition is allowed. Audited
-- to admin_order_audit. The existing 4-arg call sites stay valid because
-- the new parameter has DEFAULT false.
--
-- DROP first because PostgreSQL refuses CREATE OR REPLACE when the argument
-- list differs (5 vs 4 args). Per CLAUDE.md migration discipline. The runner
-- wraps each file in BEGIN/COMMIT so DROP + CREATE roll back atomically.

BEGIN;

DROP FUNCTION IF EXISTS public.transition_order_status(uuid, text, double precision, double precision);
...
BEGIN
  ...
  IF v_is_admin THEN
    ...
    v_valid := true;
  ELSE
    -- ── Normal paths (unchanged from 0066) ────────────────────────────────
    ...
    -- en_route → arrived (assigned washer, 100 m geofence, 4 arrival photos)

// src/components/editable/DesignEditOverlay.jsx
//
// In-app overlay shown when design edit mode is active. Listens for the
// `design-edit-open` event dispatched by <Editable>, opens a slide-in
// property editor panel that writes via `admin_set_design_override` RPC.
// A persistent "Exit edit mode" bar sits at the bottom.
...
export default function DesignEditOverlay({ app }) {
  ...
  return (
    <>
      {openId && (
        <aside
          className="fixed top-12 right-4 z-[101] w-[280px] bg-white text-zinc-900 rounded-2xl shadow-2xl border border-zinc-200 p-4"
        >
      ...
      <div
        className="fixed bottom-0 inset-x-0 z-[100] bg-amber-500 text-amber-950 text-[12px] font-semibold px-4 py-2 flex items-center justify-center gap-3 shadow-[0_-2px_8px_rgba(0,0,0,0.15)]"
      >

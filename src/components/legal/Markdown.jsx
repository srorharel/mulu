...
// **bold**, *italic*, and blank-line-separated paragraphs. Intentionally small
// — legal content authored via the admin editor uses only these constructs, and
// the repo cannot add an npm markdown dependency (registry TLS is blocked).
    ...
    <div className={className}>
      {blocks.map((raw, bi) => {
        ...
        if (lines.every(l => /^>\s?/.test(l))) {
          ...
          return (
            <blockquote key={bi} className="border-s-4 border-primary-300 dark:border-accent/40 ps-3 my-3 text-sm text-ink-muted">

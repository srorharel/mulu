...
**Addendum (2026-06-22) — "Start trip" fix + offline session hydration.** Two gaps surfaced when an underground job was actually run end-to-end:

1. **`accepted → en_route` did nothing for underground orders (bug).** `JobDrawer.advanceUnderground()` only branched on `arrived`/`in_progress`/`pending_approval`, so the **`en_route` hop fell through and "Start trip" was a no-op** — the washer could never leave the accepted screen. Fix: `advance()` routes `accepted → en_route` through the **normal server path even for underground** (`underground && trans.next !== 'en_route'` guards the offline branch). This is correct *and load-bearing*: the washer is **above ground with reception** when starting the trip, and the offline replay **requires the server to already be at `en_route`** before it will apply the arrival photos (`engine.js` arrival branch returns `'skip'` until `status === 'en_route'`). Do NOT re-route `en_route` back through the offline queue without also teaching the replay engine to perform `accepted → en_route`. Pinned by `JobDrawer.startTrip.test.jsx`.

2. **The app didn't work offline at all on a cold start (the real "can't use it underground" blocker).** If the app was killed underground and reopened with no reception, `AuthContext.fetchProfile` (→ `profiles.select`) failed → `profile` stayed null → **`RoleGuard` showed an infinite spinner** and the washer was locked out of the job they were standing next to; `get_washer_active_job` + the order fetch also need network. Fix: a tiny best-effort **localStorage read-through cache** (`src/lib/offlineCache.js`) mirrors the three session-critical objects — **profile, active job, active order** — and is used **only as a fallback when the network fetch fails** (fresh data always wins online). `fetchProfile` hydrates the cached profile (so RoleGuard passes), `Dashboard.fetchActiveJob` restores the cached active job (never *clears* on an offline error — only a successful server "no active job" clears), and `useRealtimeOrder(orderId, { cache: true })` — opt-in, enabled **only** by the washer drawer — restores the cached order row and mirrors optimistic `mutateOrder` advances so the panel shows the right status. Cache is cleared on sign-out (`clearOfflineCache`) and on terminal order status. The in-app camera already works offline (getUserMedia → RAM blob), so with profile/job/order hydrated the washer can complete the underground capture flow and it replays on resurface. **Photo capture stays in-app camera only — no gallery picker** (washers are legally barred from saving customer car/plate photos to their device; `InAppCamera.jsx`). Pinned by `offlineCache.test.js`.
...
## ADR-042: Secure clearing checkout page — env-gated, order-before-pay scaffold
**Date:** 2026-06-19
**Status:** Accepted (scaffold; live charge pending the clearing terminal)

**Context:** An Israeli card-clearing company will issue a clearing API only after the app satisfies their onboarding checklist: an About/area-of-activity page, contact (physical address **+ phone**), a תקנון covering supply / cancellation (חוק הגנת הצרכן) / age / privacy, **terms approval at the transaction**, and a **PCI-compliant payment page**. The תקנון/privacy/age/cancellation already existed (ADR-036); the real gaps were a contact phone, a transaction-time consent gate, and the payment page itself. No processor SDK exists yet.

**Decision:** New consumer route `/checkout/:id` → `src/pages/consumer/Checkout.jsx` (inside `ConsumerLayout`). The booking flow became `Home → /checkout/:id → /order/:id`: the booking CTA (relabelled "ממשיכים לתשלום", no premature "booked" toast) inserts the order (`pending`) and lands on checkout. Checkout shows the order summary + total, a **secure-payment card that hosts the clearing company's PCI iframe** (`VITE_PAYMENT_IFRAME_URL`; card data stays in the provider's scope — Privacy §2(ד)), and a **required terms-approval checkbox** (links to `/legal/{terms,privacy}`) gating the Pay button. Gated by `FEATURES.payments` (`VITE_ENABLE_PAYMENTS`, default OFF, third flag in `featureFlags.js`) — while OFF the iframe slot shows a "terminal pending" placeholder and Pay runs in scaffold mode (no charge → advances to tracking). The contact phone `050-640-6810` was added to the consumer terms + privacy policy and republished to `legal_documents` **v3** (he), live on muluwash.com + the `.md` mirrors.

**Consequence:** The order is created BEFORE payment (scaffold simplification). When the real charge is wired at the `handlePay()` integration point, listen for the provider's confirm (postMessage/redirect) and **gate washer visibility on a `paid` flag** so unpaid orders don't enter the pool. No DB schema change. Copy ran through the marketing-copy-humanizer (he/en); i18n under `consumer.checkout`. The republish used `scripts/republish-legal-phone.mjs` (guarded: publishes only if live DB still matches the pre-phone mirror). `app_config.receipt_business_phone` is a separate placeholder, intentionally not changed here.

---

## ADR-043: Saved cards (card-on-file) via processor tokenization
**Date:** 2026-06-19
**Status:** Accepted (scaffold; live token/charge calls pending the processor)

**Context:** Customers want to save a card for repeat orders. The safe, PCI-correct way is to never store the card ourselves: the clearing company keeps the card and returns a reusable **token**; we store only the token + display bits and charge by token server-side. This must not regress our "we don't store card numbers" promise (Privacy §2(ד)).

**Decision:** Migration **0128** adds `payment_methods` (`provider_token` + brand/last4/exp_month/exp_year/is_default). The token is protected at the DB layer by **column-level GRANTs**: `authenticated` may SELECT the display columns and DELETE own rows (RLS owner-scoped), but is **not granted `provider_token`** — so even the owner's browser can't read the token. Rows are written only by the service role; the default flag is flipped by `set_default_payment_method()` (SECURITY DEFINER, ownership-checked). Two Edge Functions: **`save-card`** (stores a token; client-initiated via JWT or processor-webhook via `CLEARING_WEBHOOK_SECRET`) and **`charge-saved-card`** (verifies the order + card belong to the caller, idempotent on `orders.paid_at`, charges by token). Both call the provider-agnostic **`_shared/clearing.ts`** adapter (`CLEARING_PROVIDER` selects tranzila/cardcom/payplus/generic; `'log'` default succeeds without a real call). Client: `src/lib/payments.js` (safe-columns-only reads), `useSavedCards`, a saved-card picker on the checkout page with a "save this card" consent, and a Settings → Payment methods manager (`/profile/payment-methods`). `delete-account` best-effort revokes each token then purges the rows (they also CASCADE). All gated by `VITE_ENABLE_PAYMENTS`.

**Consequence:** The PAN never touches our servers or the browser; we keep SAQ-A scope. Real tokenization/charge field-mapping is processor-specific and lives only in `_shared/clearing.ts` (marked INTEGRATION POINT). **Before enabling payments:** confirm the processor returns a reusable token + has a charge-by-token API, do 3-D Secure on first save, and add a card-on-file disclosure to terms/privacy then republish. Pinned by `savedCards.contract.test.jsx`.

## ADR-044: Light is the default theme for every role (washer dark default dropped)
**Date:** 2026-06-22
**Status:** Accepted
**Context:** Since ADR-023, `display_preference` drives theme for both roles but a **role-based default** remained: `resolveTheme` returned `'dark'` for washers and `'light'` for consumers when `display_preference` was unset. The product direction is one consistent light UI as the baseline for everyone (consumer and washer), with dark as a deliberate opt-in rather than a role default.
**Decision:** The role-based default is removed. The pure rule was extracted from `useTheme()` into `src/lib/resolveTheme.js`: explicit `display_preference` ('dark'/'light') still wins; with no preference, **every role defaults to `'light'`**; a null profile still returns `null` so the caller keeps falling back to the cached/system theme during the brief pre-load window (anti-flash). The washer Settings Display control's unset fallback changed `'dark' → 'light'` so its segmented toggle reflects the real default. Dark mode remains fully available — washers opt in via Settings → Appearance. Pinned by `src/__tests__/resolveTheme.test.js`.
**Consequence:** Washers who never explicitly chose a theme (i.e. `display_preference IS NULL`) now render **light**. Washers who explicitly set `'dark'` are unchanged. New signups: `handle_new_user` inserts profiles without `display_preference`, so the value tracks the `profiles.display_preference` **column default** — migration 0006 declared it `'dark'`, but the live default appears to have drifted to `NULL` (consumers already render light, which only holds if the column default is not `'dark'`). If a future audit finds the live default is still `'dark'`, pin it to `NULL` in a follow-up migration so new users inherit the light default. This change does **not** alter any stored row.
**Alternatives considered:** Hard-reset all washers to light by backfilling `display_preference` (rejected — clobbers washers who deliberately chose dark; "default" means default, not forced). Remove dark mode entirely (rejected — the toggle and washer dark palette stay useful). Flip only the hook without touching washer Settings' fallback (rejected — the toggle would misreport "dark" while the app rendered light).

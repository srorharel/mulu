...
**Dashboard has eight tabs** (plus the Design Editor):
...
- **Receipts tab (ADR-041)** — editor for the 9 `receipt_*`/`receipts_enabled` `app_config` keys (business name, עוסק מורשה number, sender email/name, address, phone, footer, VAT rate, kill switch) + the issued-receipts list (last 100, status badges) with per-row resend via `admin_resend_receipt`. Settings are snapshotted onto each receipt at issue time — edits affect future receipts only. See the Receipts section in DATABASE.md.

...
- **Receipt PDF retention (0122):** the receipt **row is kept forever** (tax record); only the archived PDF blob is purged from the `receipts` bucket ~6 months after issue (daily `pg_cron` 03:30 UTC → Edge Fn `purge-receipt-pdfs` → nulls `pdf_path` + stamps `pdf_purged_at`; `admin_resend_receipt` regenerates it on demand). **Activated + smoke-tested 2026-06-15:** migration applied, function deployed, Vault `purge_receipt_pdfs_url` set, cron scheduled (`TRIGGER_SECRET` already project-wide). Pinned by `receiptPdfRetention.contract.test.js`.
